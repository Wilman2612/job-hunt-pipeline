// Store sobre Postgres + pgvector. Misma API que consumían fetch/score/server/learn,
// para que el resto del sistema no cambie. Estructurado en columnas + JSONB para lo anidado.
import pg from "pg";
...
// --- upsert ---
// Inserta/actualiza una oferta. Solo pisa raw_text si viene uno nuevo (COALESCE).
...
export const upsertIndex = upsertJob;  // alias de compatibilidad
...
// --- scoring ---
// Trae TODO lo necesario para puntuar (incluye raw_text). Para el modelo no se usa: corre en Node.
export async function jobsForScoring() {
  const { rows } = await q("SELECT source, ext_id, title, company, location, url, salary, posted, easy_apply, raw_text FROM jobs");
  return rows.map((r) => ({ ...r, id: r.ext_id, easyApply: r.easy_apply, raw_text: r.raw_text || "" }));
}

// --- selección para ANÁLISIS (hard-stops ya filtrados en el SELECT, como pidió the candidate) ---
// getAll = todo el pozo; getSelectable = solo lo que vale Sonnet:
//   sin analizar + elegible POR-ANUNCIO + sobre piso salarial. Geo NUNCA se bloquea por empresa.
...
// Liviano para la UI (sin raw_text ni embedding).
...
// Última decisión por oferta, mapeada por `${source}::${ext_id}`.
