...
export async function jobsForScoring({ rescore = false } = {}) {
  ...
  const { rows } = await q(`SELECT source, ext_id, title, company, location, url, salary, posted, easy_apply, raw_text, semantic FROM jobs ${filter}`);
...
export async function loadScored() {
  ...
  return rows.map((r) => ({
    ...
    semantic: r.semantic, bestFacet: r.best_facet, hasDetail: r.has_detail, salaryUsd: r.salary_usd_year,
